...
export async function POST(req: Request) {
  ...
  const [a, b] = await prisma.signUp.findMany({
    where: { id: { in: [signUpId1, signUpId2] } },
    select: { id: true, scheduleId: true, position: true },
  });

  if (!a || !b) {
    return NextResponse.json({ error: "Signups not found" }, { status: 404 });
  }

  if (a.scheduleId !== scheduleId || b.scheduleId !== scheduleId) {
    return NextResponse.json(
      { error: "Signups do not belong to schedule" },
      { status: 400 }
    );
  }

  await prisma.$transaction([
    prisma.signUp.update({ where: { id: a.id }, data: { position: b.position } }),
    prisma.signUp.update({ where: { id: b.id }, data: { position: a.position } }),
  ]);
